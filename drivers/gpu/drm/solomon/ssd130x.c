...
{
	/*
	 * Horizontal address increment, COM split, BGR sub-pixel order and 65k
	 * (RGB565) color depth at 0 degrees orientation. Rotation is not
			       ...
			       unsigned int pitch)
{
	...
	/* TEMP debug: confirm whether non-origin (y>0) damage rects arrive */
	drm_info(&ssd130x->drm, "update_rect x=%u y=%u w=%u h=%u pitch=%u\n",
		 x, y, columns, rows, pitch);
